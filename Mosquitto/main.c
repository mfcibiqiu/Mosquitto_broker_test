...
#define KOTEL_TEMP_PATH		"zavod/ceh/kotel/temp"
#define KOTEL_MAX_TEMP		30
#define KOTEL_MIN_TEMP		25
...
void on_connect(struct mosquitto* mosq, void* obj, int reason_code)
{
	...
	rc = mosquitto_subscribe(mosq, NULL, KOTEL_TEMP_PATH, 1);
	...
	rc = mosquitto_subscribe(mosq, NULL, KOTEL_VLAZH_PATH, 1);
	if (rc != MOSQ_ERR_SUCCESS) {
		fprintf(stderr, "error subscribing: %s\n", mosquitto_strerror(rc));
		/* we might as well disconnect if we were unable to subscribe */
		mosquitto_disconnect(mosq);
	...
	rc = mosquitto_subscribe(mosq, NULL, TRUBA_TEMP_PATH, 1);
	if (rc != MOSQ_ERR_SUCCESS) {
		fprintf(stderr, "error subscribing: %s\n", mosquitto_strerror(rc));
		/* we might as well disconnect if we were unable to subscribe */
		mosquitto_disconnect(mosq);
	...
	rc = mosquitto_subscribe(mosq, NULL, TRUBA_VLAZH_PATH, 1);
	if (rc != MOSQ_ERR_SUCCESS) {
		fprintf(stderr, "error subscribing: %s\n", mosquitto_strerror(rc));
		/* we might as well disconnect if we were unable to subscribe */
		mosquitto_disconnect(mosq);
...
void on_message(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	if (!strcmp(msg->topic, KOTEL_TEMP_PATH))
	{
		printf("kotel temp: %s\n", (char*)msg->payload);

		if (atof((char*)msg->payload) > KOTEL_MAX_TEMP)
		{
			int rc;
			char message[] = "ON";
			int mid = 1;
		...
		else if (atof((char*)msg->payload) < KOTEL_MIN_TEMP)
		{
			int rc;
			char message[] = "OFF";
			int mid = 1;

			printf("Kotel overcooled\n");
	...
	{
		printf("Error: Unkonown topic: %s", msg->topic);
	}
...
int main(int argc, char* argv[])
{
	...
	rc = mosquitto_connect(mosq, "192.168.96.230", 1883, 60);
	if (rc != MOSQ_ERR_SUCCESS) {
